(function () {
    ...
    function renderPrestasiPage() {
        ...
        timeline.innerHTML = achievements.map((achievement) => {
            const link = safeLink(achievement.link);
            const content = `
            ...
            <div class="prestasi-item reveal">
                ...
                ${link
                    ? `<a class="prestasi-link" href="${link}" target="_blank" rel="noopener noreferrer" aria-label="Buka tautan ${achievement.judul}">${content}</a>`
                    : content}
            </div>
        `;
        }).join("");
